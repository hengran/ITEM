...
def get_relevance_passages_non(relevance_generation, passages, labels):
...
def main_list(file_type, llm, tokenizer, instruct, types, number, sampling_params, answer_number, nw):
    ...
    with open('work/data/non_factoid_trec1920.json') as f:
    ...
    path = 'level4_relevance_upedate/'+file_type+'/listwise/'
    ...
    try:
        for sample in tqdm(all_data[begin:len(all_data)], desc="Filename: %s" % args.outfile):
            ...
            all_num += 1
            temp_passages, ress, model_out_label = [], [], []
            ...
            while (i_round < 6):
                ...
                messages_relevance = get_prompt_relevance(question, passages, answer_generation)
                # messages_relevance = get_prompt_relevance(question, passages, answer_generation)
                ...
                if question in data_non_fact:
                    passages, labels = get_relevance_passages_non(relevance_generation, passages, labels)  
                else:
                ...
                ress.append(res)
                response = clean_response(res, len(passages))
                if isinstance(response,list):
                    response = [int(x)-1 for x in response]
                    response = remove_duplicate(response)
                else:
            ...
            outfile.write(json.dumps({
                ...
                "ground_truth_label": total_labels,
                "i_round": i_round,
...
if __name__ == '__main__':
    ...
    """
    main_list("trec", llm, tokenizer, instruct, "-iter-merge2-", 0, sampling_params, -1, 0)
